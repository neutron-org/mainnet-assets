...
- Chain upgrade point: `May 23rd 2024, 14:30 UTC` approximately at height `10,525,000`;
- Go version: `v1.21`
...
The upgrade will take place approximately on May 23rd at `14:30 UTC` approximately at height `10525000`;
...
The Neutron mainnet network, `neutron-1`, is currently running [Neutron v3.0.2](https://github.com/neutron-org/neutron/releases/tag/v3.0.2). We anticipate that operators who are running on v3.0.2, will be able to upgrade successfully. Validators are expected to ensure that their systems are up-to-date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.21`.

### Target runtime

The Neutron mainnet network, `neutron-1`, will run [Neutron v3.0.5](https://github.com/neutron-org/neutron/releases/tag/v3.0.5). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
Make sure Neutron v3.0.5 is installed by either downloading a [compatible binary](https://github.com/neutron-org/neutron/releases/tag/v3.0.5), or building from source. Building from source requires **Golang 1.21.x**.

Run Neutron v3.0.2 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v3.0.5" NEEDED at height: 10525000: upgrade to v3.0.5 and applying upgrade "v3.0.5" at height: 10525000
```
...
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v3.0.5/bin
```
...
create a cosmovisor folder inside neutron home and move neutrond v3.0.2 into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
> neutrond start --unsafe-skip-upgrade 10525000
