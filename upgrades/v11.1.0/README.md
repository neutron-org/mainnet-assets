...
- Chain upgrade point: `July 1st 2026, 15:00 UTC (approximately)`, at height `60580000`;
- Go version: `v1.24.10`
- Release: https://github.com/neutron-org/neutron/releases/tag/v11.1.0

This document describes the steps for validators and full node operators, to upgrade successfully to the Neutron v11.1.0 release. For more details on the release, please see the [release notes](https://github.com/neutron-org/neutron/releases/tag/v11.1.0).

## Upgrade date

The upgrade will take place approximately on July 1st at approximately `15:00 UTC` at height `60580000`;
...
The Neutron mainnet network, `neutron-1`, is currently running [Neutron v11.0.0](https://github.com/neutron-org/neutron/releases/tag/v11.0.0). We anticipate that operators who are running on v11.0.0, will be able to upgrade successfully. Validators are expected to ensure that their systems are up-to-date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.24`.

### Target runtime

The Neutron mainnet network, `neutron-1`, will run [Neutron v11.1.0](https://github.com/neutron-org/neutron/releases/tag/v11.1.0). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
## Create the updated Neutron binary of v11.1.0
...
### Or check checksum of the binary if you decided to [download it](https://github.com/neutron-org/neutron/releases/tag/v11.1.0)
...
- Use a statically built Neutrond binary from an official Neutron release: [https://github.com/neutron-org/neutron/releases/tag/v11.1.0](https://github.com/neutron-org/neutron/releases/tag/v11.1.0)
...
Make sure Neutron v11.1.0 is installed by either downloading a [compatible binary](https://github.com/neutron-org/neutron/releases/tag/v11.1.0), or building from source. Building from source requires **Golang 1.24.x**.

Run Neutron v11.0.0 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v11.1.0" NEEDED at height: 60580000: upgrade to v11.1.0 and applying upgrade "v11.1.0" at height: 60580000
```
...
create a Cosmovisor folder inside `$NEUTRON_HOME` and move Neutron v11.0.0 into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
build **Neutron v11.1.0**, and move neutrond v11.1.0 to `$NEUTRON_HOME/cosmovisor/upgrades/v11.1.0/bin`

```shell
mkdir -p  $NEUTRON_HOME/cosmovisor/upgrades/v11.1.0/bin
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v11.1.0/bin
```
...
> neutrond start --unsafe-skip-upgrade 60580000
