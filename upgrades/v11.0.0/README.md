---
title: Neutron v11.0.0 Upgrade
order: 2
---

<!-- markdown-link-check-disable -->

# Neutron v11.0.0 Upgrade, Instructions

- Chain upgrade point: `<TODO:date> 2026, 14:00 UTC (approximately)`, at height `<TODO:height>`;
- Go version: `v1.24.10`
- Release: https://github.com/neutron-org/neutron/releases/tag/v11.0.0
...
The upgrade will take place approximately on <TODO:date> at approximately `14:00 UTC` at height `<TODO:height>`;
...
The Neutron mainnet network, `neutron-1`, is currently running [Neutron v10.3.0](https://github.com/neutron-org/neutron/releases/tag/v10.3.0). We anticipate that operators who are running on v10.3.0, will be able to upgrade successfully. Validators are expected to ensure that their systems are up-to-date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.24`.

### Target runtime

The Neutron mainnet network, `neutron-1`, will run [Neutron v11.0.0](https://github.com/neutron-org/neutron/releases/tag/v11.0.0). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
## Create the updated Neutron binary of v11.0.0
...
### Or check checksum of the binary if you decided to [download it](https://github.com/neutron-org/neutron/releases/tag/v11.0.0)
...
- Use a statically built Neutrond binary from an official Neutron release: [https://github.com/neutron-org/neutron/releases/tag/v11.0.0](https://github.com/neutron-org/neutron/releases/tag/v11.0.0)
...
Make sure Neutron v11.0.0 is installed by either downloading a [compatible binary](https://github.com/neutron-org/neutron/releases/tag/v11.0.0), or building from source. Building from source requires **Golang 1.24.x**.

Run Neutron v10.3.0 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v11.0.0" NEEDED at height: <TODO:height>: upgrade to v11.0.0 and applying upgrade "v11.0.0" at height: <TODO:height>
```

Stop the node, and switch the binary to **Neutron v11.0.0** and re-start by `neutrond start`.
...
create a Cosmovisor folder inside `$NEUTRON_HOME` and move Neutron v10.3.0 into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
build **Neutron v11.0.0**, and move neutrond v11.0.0 to `$NEUTRON_HOME/cosmovisor/upgrades/v11.0.0/bin`

```shell
mkdir -p  $NEUTRON_HOME/cosmovisor/upgrades/v11.0.0/bin
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v11.0.0/bin
```
...
Steps to skip this upgrade proposal are simply to resume the neutron-1 network with the (downgraded) v10.3.0 binary using the following command:

> neutrond start --unsafe-skip-upgrade <TODO:height>
