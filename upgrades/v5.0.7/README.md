...
- Chain upgrade point: at height `19660000`, `February 4th 2025, 17:00 UTC` (approximately);
- Go version: `v1.22`
- Release: https://github.com/neutron-org/neutron/releases/tag/v5.0.7

This document describes the steps for validators and full node operators, to upgrade successfully to the Neutron v5.0.7 release. For more details on the release, please see the [release notes](https://github.com/neutron-org/neutron/releases/tag/v5.0.7).

## Upgrade date

The upgrade will take place approximately on February 4th at `17:00 UTC` at height `19660000`;
...
The Neutron mainnet network, `neutron-1`, will run [Neutron v5.0.7](https://github.com/neutron-org/neutron/releases/tag/v5.0.7). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
## Create the updated Neutron binary of v5.0.7
...
```shell
   $ neutrond version --long
    commit: 7b84ed7304977c9980adecb3ffdee15325108bc2
...
184d653063f68827de63ff55773804fb772100e35e30ac2ef964559b7032d639  neutrond-linux-amd64
```
...
Make sure Neutron v5.0.7 is installed by either downloading a [compatible binary](https://github.com/neutron-org/neutron/releases/tag/v5.0.7), or building from source. Building from source requires **Golang 1.22.x**.

Run Neutron v5.0.6 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v5.0.7" NEEDED at height: 19660000: upgrade to v5.0.7 and applying upgrade "v5.0.7" at height: 19660000
```

Stop the node, and switch the binary to **Neutron v5.0.7** and re-start by `neutrond start`.
...
build **Neutron v5.0.7**, and move neutrond v5.0.7 to `$NEUTRON_HOME/cosmovisor/upgrades/v5.0.7/bin`

```shell
mkdir -p  $NEUTRON_HOME/cosmovisor/upgrades/v5.0.7/bin
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v5.0.7/bin
```
...
Steps to skip this upgrade proposal are simply to resume the neutron-1 network with the (downgraded) v5.0.6 binary using the following command:

> neutrond start --unsafe-skip-upgrade 19660000
