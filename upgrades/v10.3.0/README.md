...
- Chain upgrade point: `April 15th 2026, 14:00 UTC (approximately)`, at height `53727500`;
- Go version: `v1.24.10`
- Release: https://github.com/neutron-org/neutron/releases/tag/v10.3.0

This document describes the steps for validators and full node operators, to upgrade successfully to the Neutron v10.3.0 release. For more details on the release, please see the [release notes](https://github.com/neutron-org/neutron/releases/tag/v10.3.0).

## Upgrade date

The upgrade will take place approximately on April 15th at approximately `14:00 UTC` at height `53727500`;
...
The Neutron mainnet network, `neutron-1`, is currently running [Neutron v10.2.0](https://github.com/neutron-org/neutron/releases/tag/v10.2.0). We anticipate that operators who are running on v10.2.0, will be able to upgrade successfully. Validators are expected to ensure that their systems are up-to-date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.24`.

### Target runtime

The Neutron mainnet network, `neutron-1`, will run [Neutron v10.3.0](https://github.com/neutron-org/neutron/releases/tag/v10.3.0). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
## Create the updated Neutron binary of v10.3.0
...
### Or check checksum of the binary if you decided to [download it](https://github.com/neutron-org/neutron/releases/tag/v10.3.0)
...
- Use a statically built Neutrond binary from an official Neutron release: [https://github.com/neutron-org/neutron/releases/tag/v10.3.0](https://github.com/neutron-org/neutron/releases/tag/v10.3.0)
...
Make sure Neutron v10.3.0 is installed by either downloading a [compatible binary](https://github.com/neutron-org/neutron/releases/tag/v10.3.0), or building from source. Building from source requires **Golang 1.24.x**.

Run Neutron v10.2.0 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v10.3.0" NEEDED at height: 53727500: upgrade to v10.3.0 and applying upgrade "v10.3.0" at height: 53727500
```

Stop the node, and switch the binary to **Neutron v10.3.0** and re-start by `neutrond start`.
...
create a Cosmovisor folder inside `$NEUTRON_HOME` and move Neutron v10.2.0 into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
build **Neutron v10.3.0**, and move neutrond v10.3.0 to `$NEUTRON_HOME/cosmovisor/upgrades/v10.3.0/bin`

```shell
mkdir -p  $NEUTRON_HOME/cosmovisor/upgrades/v10.3.0/bin
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v10.3.0/bin
```
...
Steps to skip this upgrade proposal are simply to resume the neutron-1 network with the (downgraded) v10.2.0 binary using the following command:

> neutrond start --unsafe-skip-upgrade 53727500
