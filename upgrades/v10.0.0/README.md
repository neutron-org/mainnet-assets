...
The upgrade will take place approximately on March 5th at approximately `15:00 UTC` at height `50040000`;
...
The Neutron mainnet network, `neutron-1`, will run [Neutron v10.0.0](https://github.com/neutron-org/neutron/releases/tag/v10.0.0). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
## Create the updated Neutron binary of v10.0.0
...
### Or check checksum of the binary if you decided to [download it](https://github.com/neutron-org/neutron/releases/tag/v10.0.0)
...
- Use a statically built Neutrond binary from an official Neutron release: [https://github.com/neutron-org/neutron/releases/tag/v10.0.0](https://github.com/neutron-org/neutron/releases/tag/v10.0.0)
...
```shell
ERR UPGRADE "v10.0.0" NEEDED at height: 50040000: upgrade to v10.0.0 and applying upgrade "v10.0.0" at height: 50040000
```

Stop the node, and switch the binary to **Neutron v10.0.0** and re-start by `neutrond start`.
...
create a Cosmovisor folder inside `$NEUTRON_HOME` and move Neutron v9.1.3 into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
build **Neutron v10.0.0**, and move neutrond v10.0.0 to `$NEUTRON_HOME/cosmovisor/upgrades/v10.0.0/bin`

```shell
mkdir -p  $NEUTRON_HOME/cosmovisor/upgrades/v10.0.0/bin
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v10.0.0/bin
```
...
Steps to skip this upgrade proposal are simply to resume the neutron-1 network with the (downgraded) v9.1.3 binary using the following command:

> neutrond start --unsafe-skip-upgrade 50040000
